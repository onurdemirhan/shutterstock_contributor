...
for sno in no_list:
    ...
    try:
        with requests.get(
                        "https://www.shutterstock.com/_next/data/NZRov-ntnEVhE7744I6Xk/en/_shutterstock/search/" + sno + ".json?term=" + sno, headers=headers) as url:
                    data = url.json()
    ...
    with requests.get(
            "https://www.shutterstock.com/_next/data/NZRov-ntnEVhE7744I6Xk/en/_shutterstock" + new_url + ".json?title=" + new_url.strip("/image-photo/"), headers=headers) as url:
            data = url.json()
